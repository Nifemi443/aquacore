    ...
    <div className="flex items-start justify-between gap-4 py-4 first:pt-0 last:pb-0">
      ...
      <button
        ...
        className={`flex h-7 w-12 shrink-0 items-center rounded-full p-0.5 transition-colors duration-200 ${
          checked ? "justify-end bg-[var(--color-accent)]" : "justify-start bg-neutral-200"
        }`}
      >
        <span className="h-6 w-6 shrink-0 rounded-full bg-white shadow-sm" />
      </button>
    </div>
  );
}
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[var(--color-surface)] pb-24 text-[var(--color-text-primary)] lg:pb-0">
      ...
      <div className="min-w-0 overflow-x-hidden lg:pl-[256px]">
        <AppMobileHeader activeKey="settings" />
      ...
      <AppMobileNav activeKey="settings" />
    </main>
  );
}
